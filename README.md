...
| 模块 | 职责 |
|------|------|
| `src/db/index.ts` | Dexie 实例、5 张表 schema（v1~v3 升级）、`initDb` 初始化 |
...
| 表 | 主键 | 索引字段 | 说明 |
|----|------|---------|------|
| students | id | name, total_points | 宝贝（三指标/星星/等级/经验） |
...
│   │   ├── PetImage.vue         # 宠物图片组件
│   │   └── home/                # 主页面组件
│   │       ├── TopBar.vue       # 顶部导航（宝贝/排行/更多）
│   │       ├── StudentCard.vue  # 宝贝卡片（三指标/星星/休眠角标）
│   │       ├── StudentDetailPanel.vue # 宝贝详情（状态/评价/商城/记录）
│   │       ├── ShopModal.vue    # 宠物商城弹窗
│   │       ├── RulesModal.vue   # 规则管理（星星评分）
│   │       ├── RankingModal.vue # 排行榜
